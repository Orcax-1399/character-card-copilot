...
import { testApiConnection } from '@/services/apiConfig';
...
function handleSetDefault() {
  if (!props.api.default) {
    emit('setDefault', props.api.profile);
  }
}
...
async function handleTestConnection() {
  if (!props.api.endpoint || !props.api.key) {
    const result: ApiTestResult = {
      success: false,
      message: '请先配置API端点和密钥',
      error: 'Missing endpoint or key'
    };
  ...
  try {
    const result = await testApiConnection(props.api);
    testResult.value = result;
    emit('testConnection', result);
    return result;
  } catch (error) {
    ...
    <div class="flex items-center justify-between">
      <div class="flex items-center gap-2">
        ...
        <div class="flex-1 min-w-0">
          <div class="flex items-center gap-2">
            <h3 class="font-medium text-gray-800 text-sm truncate">{{ api.profile }}</h3>
            <span
              v-if="api.default"
