  ...
  <div class="space-y-3">
    ...
    <div class="flex gap-2">
      ...
      <div class="relative">
        ...
        >
          <div>
            <label class="text-sm font-semibold text-gray-700 mb-2 block">显示状态</label>
            <div class="space-y-2">
              <label class="flex items-center gap-2 cursor-pointer">
                ...
                />
                <span class="text-sm text-gray-700">显示启用的条目</span>
              </label>
              <label class="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  class="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                />
                <span class="text-sm text-gray-700">显示禁用的条目</span>
              </label>
            </div>
          </div>

          <div class="border-t border-gray-200 pt-3">
            <label class="text-sm font-semibold text-gray-700 mb-2 block">排序方式</label>
            <select
              v-model="sortBy"
          ...
          <div>
            <label class="text-sm font-semibold text-gray-700 mb-2 block">排序方向</label>
            <select
              v-model="sortOrder"
              class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
              @change="handleSortChange"
      ...
      <!-- 新建条目按钮 -->
      <button
        class="bg-blue-500 hover:bg-blue-700 text-white text-sm font-medium py-2 px-3 rounded-full flex items-center gap-1.5"
        @click="$emit('create')"
      >
      ...
      <div class="flex items-center justify-between text-sm">
        <div class="flex items-center gap-4">
          <span class="text-gray-600">
            共 <span class="font-bold text-gray-900">{{ statistics.total }}</span> 个条目
          </span>
          <span class="text-green-600">
            启用: <span class="font-bold">{{ statistics.enabled }}</span>
          </span>
          <span class="text-gray-500">
            禁用: <span class="font-bold">{{ statistics.disabled }}</span>
          </span>
        </div>
        <div v-if="filteredCount !== statistics.total" class="text-gray-600">
          筛选结果: <span class="font-bold text-blue-600">{{ filteredCount }}</span>
        </div>
      </div>
    </div>
  </div>
